...
def train_one_epoch(model, model_without_ddp, data_loader, optimizer, device, epoch, log_writer=None, args=None):
    ...
    for data_iter_step, batch in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.amp.autocast('cuda', dtype=torch.bfloat16):
            loss_jit, loss_mta = model(x, labels)
        loss_value_jit = loss_jit.item()
        loss_value_mta = loss_mta.item()

        loss = loss_jit + 0.1 * loss_mta
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        loss_value_jit_reduce = misc.all_reduce_mean(loss_value_jit)
        loss_value_mta_reduce = misc.all_reduce_mean(loss_value_mta)

        if log_writer is not None:
            # Use epoch_1000x as the x-axis in TensorBoard to calibrate curves.
            epoch_1000x = int((data_iter_step / len(data_loader) + epoch) * 1000)
            if data_iter_step % args.log_freq == 0:
                log_writer.add_scalar('train_loss_jit', loss_value_jit_reduce, epoch_1000x)
...
def evaluate(model_without_ddp, args, epoch, batch_size=64, log_writer=None):
    ...
    model_state_dict = copy.deepcopy(model_without_ddp.net.state_dict())
    ema_state_dict = copy.deepcopy(model_without_ddp.net.state_dict())

    for i, (name, _value) in enumerate(model_without_ddp.net.named_parameters()):
        assert name in ema_state_dict
        ema_state_dict[name] = model_without_ddp.ema_params1[i]
    
    print("Switch to ema (Net only)")
    model_without_ddp.net.load_state_dict(ema_state_dict)
    ...
    model_without_ddp.net.load_state_dict(model_state_dict)
