...
class MaskedTransformerAdapter(nn.Module):
    def __init__(
        self,
        input_size=256,
        patch_size=16,
        ...
        self.projection_layer = ProjectionHead(hidden_size, self.out_channels)
...
class JiT(nn.Module):
    ...
    def forward_encoder(self, x, t, y, mask=None):
        """
        x: (N, C, H, W)
        t: (N,)
        y: (N,)
        ...
        for i, block in enumerate(self.blocks[:self.in_context_start]):
            x = block(x, c, self.feat_rope)

        return x
    
    def forward_decoder(self, x, t, y, mask=None):
        """
        x: (N, C, H, W)
        t: (N,)
        y: (N,)
...
def MTA_B_16(**kwargs):
    return MaskedTransformerAdapter(depth=2, hidden_size=768, num_heads=12, patch_size=16, in_context_len=32, **kwargs)

def MTA_L_16(**kwargs):
    return MaskedTransformerAdapter(depth=2, hidden_size=1024, num_heads=16, patch_size=16, in_context_len=32, **kwargs)

def MTA_H_16(**kwargs):
    return MaskedTransformerAdapter(depth=2, hidden_size=1280, num_heads=16, patch_size=16, in_context_len=32, **kwargs)
...
MaskedTransformerAdapter_models = {
    'MTA-B/16': MTA_B_16,
    'MTA-L/16': MTA_L_16,
    'MTA-H/16': MTA_H_16,
}